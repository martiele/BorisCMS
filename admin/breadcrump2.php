...
function breadcrump($ambito, $tipo, $val){
	...
	echo '	<ul id="crumbs">';
	echo '		<li><a href="gruppi_gest.php">Soglie</a></li>';
	$lista = "";
	switch($ambito){
		case "soglie":
			...
			if($tipo=="gest"){
				...
				if($tipo=="add"){
					$lista = sprintf('<li>Nuova Soglia</a></li>') . $lista;
				}
				if($tipo=="edit"){
					$lista = sprintf('<li>Modifica Soglia</a></li>') . $lista;
				}
